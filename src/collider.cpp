...
Collider::Collider(const Object &parent, std::unique_ptr<AABB> aabb)
  ...
  println("collider {:x}", reinterpret_cast<size_t>(this));
}
...
Convex::Convex(const Object &parent, std::vector<glm::vec2> &&vertices)
  ...
  println("convex {:x}", reinterpret_cast<size_t>(this));

  std::ranges::for_each(
      this->vertices,
      [&aabb = this->aabb](const glm::vec2 &v) { aabb->expand(v); });
}
Convex Convex::create(const Object &parent, const unsigned char n,
                      const double radius, const double offset) {
  return {parent, ngonVertices(n, radius, offset)};
}
...
Circle::Circle(const Object &parent, const double radius)
    : Collider(parent,
      ...
      radius{radius} {}
Circle Circle::create(const Object &parent, const double radius) {
